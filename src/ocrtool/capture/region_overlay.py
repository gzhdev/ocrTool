...
class RegionOverlay(QWidget):
    ...
    selectionDone = Signal(QRect)  # 逻辑坐标选区（已夹取在本屏内）
    cancelled = Signal()
    ...
    def _paint_size_hint(self, painter: QPainter, selection: QRect) -> None:
        physical_width, physical_height = physical_size_of(selection, self._snapshot)
    ...
    def mouseReleaseEvent(self, event) -> None:  # noqa: N802
        ...
        if physical_width < MIN_SELECTION_PX or physical_height < MIN_SELECTION_PX:
            ...
            self.cancelled.emit()
            return
        self.selectionDone.emit(selection)
...
class RegionCaptureFlow(QObject):
    ...
    def start(self, window: QWidget | None) -> None:
        ...
        self._window = window
        try:
            self._window_was_visible = self._hide_and_wait(window)
    ...
    def _hide_and_wait(self, window: QWidget | None) -> bool:
        """隐藏主窗口并等待实际生效；返回发起前是否可见。
    ...
    def _show_overlays(self, snapshots: list[ScreenSnapshot]) -> None:
        ...
        for snapshot in snapshots:
            ...
            overlay.selectionDone.connect(
                lambda rect, snap=snapshot: self._on_selection_done(snap, rect)
            )
    ...
    def _on_selection_done(self, snapshot: ScreenSnapshot, rect: QRect) -> None:
        if self._ended:
            return
        try:
