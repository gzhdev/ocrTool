...
设计要点（详见 `openspec/specs/screen-capture/spec.md`）：

- **冻结帧选区**：进入流程立即完成所有屏幕的像素捕获，选区在静态画面上进行。
  选区期间屏幕内容变化（视频、通知、光标闪烁）不影响结果——「看到的」与「截到的」
  是同一帧，裁剪是纯内存操作，不存在二次捕获的时间差。
- **多显示器与 DPI（逐屏捕获）**：每块屏幕单独捕获、单独创建覆盖层，坐标换算用
  **该屏自身**的 `devicePixelRatio`（逻辑选区 → 屏内局部 → ×该屏比例 → 物理像素矩形）。
  整虚拟桌面单次捕获只携带一个比例，混合 DPI（如 150% + 100%）下必然有一块屏错位；
  该类缺陷不崩溃、只「识别到没框选的区域」，是本功能的首要风险，坐标正确性已按
  决策关卡在 100% 与非 100% 缩放下逐像素验证。
- **选区不跨屏**：一次选区限定在按下起点所在的屏幕内，拖出边界即在该屏边界截止。
  跨屏拼接会产生内含两种像素密度的图像（同字号文字大小不一），直接损害识别质量，
  属于「实现了也是劣化的」能力，明确放弃。
- **主窗口隐藏与恢复**：发起截图先隐藏主窗口，以「事件循环确认窗口不再暴露 +
  200ms 下限延时」双条件等待隐藏实际生效后才捕获（纯固定延时在慢机器上仍会截到
  残影）；流程以任何方式结束都恢复主窗口到发起前的可见状态。
- **取消语义**：ESC、右键、面积过小（任一边 < 8 物理像素，含误触单击）均按取消
  处理——不报错、不弹窗、不产生识别请求，当前图像与结果保持不变。
- **覆盖层生命周期**：所有结束路径（完成、取消、异常）汇聚到单一收尾出口，统一
  销毁全部屏幕的覆盖层并恢复主窗口——任何路径遗留全屏置顶窗口都只能靠强杀进程
  解决，用结构而非纪律保证其不发生。
- **第四种输入**：捕获图像与打开文件 / 拖放 / 粘贴走同一识别管线（含最大边长
  等比缩放与 `scale` 携带），但跳过扩展名与可解码校验（程序自身生成的内存图像，
  格式校验不适用）；完成选区自动发起识别，全程不产生任何磁盘文件。
- 应用内快捷键 `Ctrl+Shift+S`；远程桌面等环境捕获返回空图像时给出可读提示而非崩溃。
...
## 36.1 单实例（全局快捷键 / 开机启动 / 系统托盘 的共同前提）

全局快捷键、开机启动、系统托盘不是三个独立功能，而是「应用从前台程序
变为后台常驻进程」这一架构变更的三个侧面，三者互为前提。而常驻引入一个
必须先建立的前提：**单实例**——否则开机自启的托盘实例与用户双击的第二个
实例并存，快捷键被其一占用、托盘出现两个图标，用户只会觉得「时灵时不灵」。

实现要点（openspec: background-residency）：

- **实例端点**：本地套接字（Windows 命名管道）承担检测与通信，端点名称由
  当前用户与程序所在目录共同派生——多用户互不干扰，不同目录的便携副本可
  同时运行；
- **陈旧端点清理（关键）**：进程被强杀或崩溃后，端点不会被系统自动回收
  （Unix 套接字文件残留；Windows 命名管道虽随进程回收，但保持同一清理
  调用以统一语义）。若不主动清理，下次启动会误判「已有实例在运行」而
  **永远无法启动**——一次偶发崩溃升级为永久性故障，用户完全无从排查。
  因此启动序列固定为：先连接探测（连接成功 = 已有实例，激活其窗口后本次
  退出）→ 连不上则显式移除同名遗留端点 → 再建立监听；
- **互斥量仲裁**：毫秒级并发双启动时，双方连接探测都落空、双方监听同名
  命名管道都可能成功（Windows 管道允许多实例）。命名互斥量
  （`Local\<端点名>`）提供先到先得的原子仲裁——抢不到即按已有实例退出，
  进程终止时由系统回收，无陈旧残留问题。
...
## 43.1 自身目录之外的持久化痕迹（注册表登录启动项）

开启「开机启动」后，程序会在**当前用户的注册表**写入一条登录启动项——
这是本程序第一处落在 `APP_ROOT` / `USER_ROOT` 双根目录之外的持久化痕迹，
用户须知：

- **位置**：`HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Run`
- **值名**：`OCRTool`；值为带引号的程序完整路径加 `--tray` 参数
- **手动移除**：在本程序托盘菜单关闭「开机启动」，或用注册表编辑器
  （`regedit`）删除上述值；亦可用系统「任务管理器 → 启动应用」禁用
- **已知限制**：直接删除程序目录后，该启动项无法被自动清理（程序已不
  存在，也没有卸载器）。此残留指向不存在的文件，系统登录时会忽略它，
  无实际危害，但会遗留一条注册表记录
- **路径自愈**：每次启动时若该功能开启，用当前实际路径重写启动项——
  便携目录被移动后自启仍然有效；路径未变化时不产生多余写入
